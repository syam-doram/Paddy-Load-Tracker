...
export default function App() {
  ...
  const [view, setView] = useState<'dashboard' | 'lorry-detail' | 'new-lorry' | 'analytics' | 'labour-management' | 'market' | 'market-entry' | 'login' | 'register' | 'onboarding'>(() => {
    const saved = localStorage.getItem('paddy_user');
...
const today = new Date().toISOString().split("T")[0];
  ...
  // Form states
  const [newLorryForm, setNewLorryForm] = useState({ lorry_number: '', driver_name: '' });
  ...
  const handleCreateLorry = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await fetch(getApiUrl('/api/lorries'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...newLorryForm, date: selectedDate })
      });
      const data = await res.json();
      setSelectedLorryId(data.id);
      setView('lorry-detail');
      setNewLorryForm({ lorry_number: '', driver_name: '' });
      fetchLorries();
      fetchStats();
    } catch (err) {
      console.error("Failed to create lorry", err);
    }
  };
          ...
          <div className="max-w-5xl mx-auto flex justify-between items-center">
            ...
            <nav className="hidden md:flex items-center gap-1 bg-zinc-100 p-1 rounded-lg border border-zinc-200">
              <button 
                ...
                className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all flex items-center gap-2 ${
                  view === 'dashboard' || view === 'lorry-detail' || view === 'new-lorry'
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="p-2 rounded-md text-zinc-500 hover:text-zinc-700 transition-colors"
              >
      ...
      <AnimatePresence>
        {sideOpen && (
          <motion.aside
            ...
            className="fixed left-0 top-0 h-full w-72 bg-zinc-900 border-r border-zinc-800 z-40 flex flex-col shadow-2xl"
          ...
          >
            ...
            <div className="px-6 py-8 border-t border-zinc-800 flex flex-col items-center">
    ...
    <div className="flex justify-between items-end">
      <div>
        <h2 className="text-2xl font-bold text-zinc-900">
          {isToday ? "Today's Lorries" : `Records for ${selectedDate}`}
        </h2>
        <p className="text-zinc-500">
          Manage active loads and historical records.
        </p>
      </div>
                      ...
                      {Array.isArray(stats.paddyTypeStats) && stats.paddyTypeStats.length > 0 ? (
                        <ResponsiveContainer width="100%" height={360}>
                          {(() => {
                            ...
                            const paddyTypes = Array.from(new Set(stats.paddyTypeStats.map(row => row.paddy_type || 'Unknown')));
                            // 3. Build chart data: [{ date, [paddyType1]: bags, ... }]
                            // Since paddyTypeStats has no date, sum all bags for each paddy type in the last 7 days
                            // If you want to filter by area, add area logic here
                            const chartData = weekDates.map(date => {
                              const row: any = { date };
                              paddyTypes.forEach(pt => {
                                // For each paddy type, sum bags from paddyTypeStats (no date info, so show total)
                                row[pt] = stats.paddyTypeStats
                                  .filter(r => (r.paddy_type || 'Unknown') === pt)
                                  .reduce((sum, r) => sum + (Number(r.bags) || 0), 0);
                              });
                              return row;
                            });
                            const colorList = [
                              '#34d399', '#06b6d4', '#fbbf24', '#f472b6', '#818cf8', '#f87171', '#4ade80', '#facc15', '#a3e635', '#f472b6'
                            ];
                            return (
                              <ComposedChart
                                data={chartData}
                              ...
                              >
                                ...
                                />
                                <Legend iconType="circle" wrapperStyle={{ fontSize: 14, marginBottom: 0 }} />
                                {paddyTypes.map((pt, idx) => (
                                  <Bar
                                    key={pt}
                                    dataKey={pt}
                                    name={pt}
          ...
          {view === 'new-lorry' && (
            ...
            >
              ...
              <div className="bg-white border border-zinc-200 rounded-2xl p-8 shadow-sm">
                <h2 className="text-2xl font-bold mb-2">New Lorry Entry</h2>
                <p className="text-zinc-500 mb-8">Enter the lorry details to start recording bag counts.</p>
                
                <form onSubmit={handleCreateLorry} className="space-y-6">
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-zinc-500 mb-2">Lorry Number</label>
                    <input 
                      required
                      type="text"
